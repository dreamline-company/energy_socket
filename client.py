...
if s == "2":
    ...
    packet = [PACKET_START_CHARACTER, OBJECT_ID, REGULAR_PACKET_TYPE, round(time.time()).to_bytes(4, "little"), DATA_START_CHARACTER]
    ...
    for name, value in dict_val.items():
        packet.extend((name.encode(), bytearray(struct.pack("f", value)) , DATA_DELIMITER))
    packet.extend((DATA_END_CHARACTER, PACKET_END_CHARACTER))
    print(packet)
    for i in packet:
        sock.send(i) 
